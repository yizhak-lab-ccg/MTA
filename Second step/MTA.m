%MTA:
%Input - 1. The Model with a given media
%                2. v_ref -  a reference vector describing the source state
%                as derived from an iMAT-based analysis (A column vector)
...
%                    target state (a row vector)
%                4. rxns_to_delete - A set of reactions to KO (a column
%                vector)
%			     5. thr - threshold defining the  minimum required change for the integer constraints
...
function[score,stat] = MTA(model,v_ref,discrete_rxns_vector,rxns_to_delete,thr)
